...
let count_atari_groups (game_state : Game_state.t) (player : Player_kind.t) : int =
  ...
  let seen = Core.Hash_set.create ~size:1000 (module struct
    include CP
  ...
  for row = 0 to 18 do
    for col = 0 to 18 do
      ...
      | Some stone when Player_kind.equal stone.owner player ->
        if not (Core.Hash_set.mem seen stone.position) then (
          ...
          let group_libs =
            Core.List.fold group ~init:0 ~f:(fun acc pos ->
              let libs = Game_state.neighbors_of pos
...
let total_liberties (game_state : Game_state.t) (player : Player_kind.t) : int =
  let module CP = Cell_position in
  let seen = Hash_set.create (module struct
    include CP
    let hash t = Hashtbl.hash t
  end) in
  ...
  for row = 0 to 18 do
    for col = 0 to 18 do
      ...
      | Some stone when Player_kind.equal stone.owner player ->
        ...
        let friendly = List.count neighbors ~f:(fun npos ->
          let r = npos.CP.row in
          let c = npos.CP.column in
...
let heuristic_value (game_state : Game_state.t) : int =
  ...
  | Decision.Winner _ -> 
    ...
    (match game_state.decision with
     ...
     | _ -> 0)  (* This case won't occur *)
  | Decision.In_progress { whose_turn } ->
    let my_libs = total_liberties game_state whose_turn in
    let opp_libs = total_liberties game_state (Player_kind.opposite whose_turn) in
    let my_captures, opp_captures =
    ...
    let weights =
      if Float.(percent_filled < 0.33) then (* Early game *)
        (1000,   (* captures *)
         500,   (* compact groups *)
         600,   (* group count *)
         0,    (* liberties *)
        ...
        -1000,    (* self atari *)
        -80)    (* long lines *)
      else if Float.(percent_filled < 0.66) then (* Mid game *)
        (1000,  (* captures *)
         200,   (* compact groups *)
         200,   (* group count *)
         500,   (* liberties *)
        ...
        -80)    (* long lines *)
    in
    let w_captures, w_compact, w_groups, w_libs, w_atari, w_self_atari, w_lines = weights in
    w_captures * (my_captures - opp_captures)
    + w_compact * compact
    + w_groups * group_count
    + w_libs * (my_libs - opp_libs)
    + w_atari * opp_atari
    + w_self_atari * self_atari
    + w_lines * long_lines
;;

let is_capture (parent : Game_state.t) (child : Game_state.t) (whose_turn : Player_kind.t) : bool =
  match whose_turn with
  | Player_kind.Black ->
    child.black_captures > parent.black_captures
  | Player_kind.White ->
    child.white_captures > parent.white_captures
;;
...
let best_move (game_state : Game_state.t) : Move.t option =
  ...
  | Decision.In_progress { whose_turn } ->
    ...
    if filled_cells = 0 then
      Some (Move.Place { Cell_position.row = 9; column = 9 })
    else (
      ...
      in
      let candidate_moves = if not (List.is_empty priority_moves) then priority_moves else moves_and_children in
      ...
      in
      let best_score =
        List.max_elt moves_and_values ~compare:(fun (_, v1) (_, v2) -> Int.compare v1 v2) |> Option.map ~f:snd
      in
      match best_score with
      | None -> None
      | Some score ->
        let best_moves = List.filter moves_and_values ~f:(fun (_move, value) -> value = score) in
...
let _children (game_state : Game_state.t) ~(sort_by_whose_turn : Player_kind.t) : Game_state.t list =
  let compare =
